...
float DegreesPerLED = (float)NUM_LEDS / 360.0;
...
void FillLEDsFromPaletteColors( uint8_t colorIndex)
{
  ...
  uint8_t tempColorIndex = colorIndex;
  
  for( int i = 0; i < NUM_LEDS / 2; i++) {
    leds[i] = ColorFromPalette( currentPalette, tempColorIndex, quadwave8((colorIndex << 1) + (i << 5)), currentBlending);
    tempColorIndex += 3;
  }
  tempColorIndex = colorIndex;
  for( int i = 0; i < NUM_LEDS / 2; i++) {
    leds[NUM_LEDS - 1 - i] = ColorFromPalette( currentPalette, tempColorIndex, quadwave8((colorIndex << 1) + (i << 5)), currentBlending);
    tempColorIndex += 3;
  }
}
